 ...
 * =============================================================================
 */
import { util, clone, env, scalar, keep, tensor, stack, concat, unstack, tidy, reshape, slice, io, Tensor } from '@tensorflow/tfjs-core';
import { squaredDifference, pow, maximum, minimum, sub, floorDiv, divNoNan, div, mul, mod, addN, add, isNaN, prelu, leakyRelu, prod, rsqrt, relu6, clipByValue, tan, tanh, square, sqrt, softplus, sinh, sign, sin, sigmoid, selu, round, relu, real, reciprocal, neg, imag, log1p, log, floor, expm1, exp, erf, elu, cosh, cos, complex, ceil, atanh, atan2, atan, asinh, asin, acosh, acos, abs, dilation2d, maxPool3d, avgPool3d, maxPoolWithArgmax, maxPool, avgPool, conv3d, depthwiseConv2d, conv2dTranspose, fused, conv2d, conv1d, zerosLike, zeros, truncatedNormal, range, randomUniform, onesLike, ones, oneHot, multinomial, linspace, fill, setdiff1dAsync, cast, whereAsync, image as image$1, unique, topk, scalar as scalar$1, tensor1d, where, logicalOr, logicalNot, logicalAnd, lessEqual, less, greaterEqual, greater, notEqual, equal, transpose, einsum, matMul, sparseToDense, logSoftmax, softmax, localResponseNormalization, batchNorm, denseBincount, bincount, cumsum, argMin, argMax, any, all, sum, min, mean, max, gatherND, scatterND, split as split$1, tile, unstack as unstack$1, squeeze, reshape as reshape$1, stack as stack$1, stridedSlice, slice as slice$1, reverse, gather, concat as concat$1, sparse as sparse$1, irfft, rfft, ifft, fft, string as string$1, broadcastTo, depthToSpace, batchToSpaceND, spaceToBatchND, pad, mirrorPad, expandDims } from '@tensorflow/tfjs-core/dist/ops/ops_for_converter';
...
const executeOp$7 = (node, tensorMap, context) => {
    switch (node.op) {
        ...
        case 'Size':
            return [scalar$1(getParamValue('x', node, tensorMap, context).size, 'int32')];
        case 'Rank':
            return [scalar$1(getParamValue('x', node, tensorMap, context).rank, 'int32')];
        case 'NoOp':
            return [scalar$1(1)];
        case 'Print':
...
class HashTable {
     ...
     */
    tensorSize() {
        return scalar$1(this.size(), 'int32');
    }
    /**
...
const executeOp$e = (node, tensorMap, context) => {
    switch (node.op) {
        ...
        case 'Slice': {
            ...
            // tslint:disable-next-line:no-any
            const size = getParamValue('size', node, tensorMap, context);
            return [slice$1(getParamValue('x', node, tensorMap, context), begin, size)];
        }
        case 'StridedSlice': {
            const begin = getParamValue('begin', node, tensorMap, context);
        ...
        case 'Pack': {
            return tidy(() => {
                ...
                const mapped = tensors.map(tensor => {
                    ...
                    return sameShape ? tensor : reshape$1(tensor, shape);
                });
                return [stack$1(mapped, axis)];
            });
        }
        case 'Unpack': {
            const axis = getParamValue('axis', node, tensorMap, context);
            const tensor = getParamValue('tensor', node, tensorMap, context);
            return unstack$1(tensor, axis);
        }
        case 'Tile': {
...
const executeOp$i = (node, tensorMap, context) => {
    switch (node.op) {
        ...
        case 'Reshape': {
            return [reshape$1(getParamValue('x', node, tensorMap, context), getParamValue('shape', node, tensorMap, context))];
        }
        case 'MirrorPad': {
